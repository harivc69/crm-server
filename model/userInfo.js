...
const UserInfo = createModel('UserInfo');

const getUserInfoData = async (req, res) => {
  try {
    const data = await UserInfo.find(); // Retrieve all documents from the 'menu' collection
    if (!data || data.length === 0) {
      throw new Error('UserData data not found');
    }
    res.json(data);
  } catch (error) {
...
router.get('/UserInfodata', getUserInfoData);
